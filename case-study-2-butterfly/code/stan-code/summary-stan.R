# summary-stan.R: this script summarizes the results from the 
#                 butterfly case study fit with stan through the rstan
#                 package.
...
# Read in results ---------------------------------------------------------
# Note this object is not included on Github as it is too large. for regular
# storage. The results file can be generated from running the "main-stan.R" 
# script, or you can contact Jeff Doser (doserjef@msu.edu).
load("results/stan-icm-results.rda")
...
# Linear year trend
MCMCsummary(out, params = 'betaYear', exact = FALSE)
# Community-level means
MCMCsummary(out, params = 'muBeta', exact = FALSE)
...
trend.log.samples <- MCMCchains(out, params = 'betaYear', exact = FALSE)
# Community-level trend values on the log scale. 
trend.comm.log.samples <- MCMCchains(out, params = 'muBetaYear', exact = FALSE)
# Probably community level trend < 0
mean(trend.comm.log.samples < 0)

n.years <- n_distinct(covs.df$year.cov)
int.samples <- MCMCchains(out, params = 'betaInt', exact = FALSE)
int.comm.samples <- MCMCchains(out, params = 'muBetaInt', exact = FALSE)
# Extract year random effects
trend.re.samples <- MCMCchains(out, params = 'betaStarYear', exact = FALSE)
# Order: sample, years, species. 
trend.re.samples <- array(trend.re.samples, 
...
survey.samples <- MCMCchains(out, params = 'betaSurvey', exact = FALSE)
survey.comm.samples <- MCMCchains(out, params = 'muBetaSurvey', exact = FALSE)
...
# Make the plot. Note the plot shown in the manuscript is produced from the spAbundance model.
