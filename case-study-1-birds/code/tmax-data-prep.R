# tmax-pred-data-prep.R: this script extracts tmax data from PRISM
...
# 12 months in a year. 
tmax.vals <- matrix(NA, nrow = nrow(coords.albers), ncol = 12)
...
curr.year <- 2016
get_prism_monthlys(type = "tmax", year = curr.year, mon = 1:12, keepZip = FALSE)
# Get file name to data of interest
# Loop through all the months
for (i in 1:ncol(tmax.vals)) {
  print(i)
  tmax.curr.path <- prism_archive_subset("tmax", "monthly", years = curr.year, mon = i)
  # Get absolute file path
  tmax.curr.abs <- pd_to_file(tmax.curr.path)
  # Download the raster
  tmax.curr <- read_stars(tmax.curr.abs)
  # Get coordinates
  coords.proj <- coords.sf.albers %>%
    st_transform(crs = st_crs(tmax.curr))
  # Extract tmax value at the center of each 5x5km grid cell. 
  tmp <- st_extract(tmax.curr, at = coords.proj)
  tmax.vals[, i] <- tmp[[1]]
}

# Save values to hard drive -----------------------------------------------
save(tmax.vals, file = paste("data/tmax-", curr.year, ".rda", sep = ''))
